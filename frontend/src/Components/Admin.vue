...
const eventHeaders = [
  ...
  { title: 'Spots', key: 'spots_available' },
  { title: 'Price', key: 'price' },
  { title: 'Actions', key: 'actions', sortable: false, width: '120px' }
]
...
function calculateStats() {
  ...
  stats.value.upcomingEvents = events.value.filter(e => new Date(e.date) >= new Date()).length
...
function convertUnsplashUrl(url) {
  ...
  const unsplashPhotoRegex = /https?:\/\/unsplash\.com\/photos\/[^\/]+\/([a-zA-Z0-9_-]+)/
  const match = url.match(unsplashPhotoRegex)

  if (match) {
    // Extract photo ID (last part of URL)
    const photoId = match[1]
    return `https://images.unsplash.com/photo-${photoId}?w=400`
  }

  // If already a proper image URL, return as is
  return url
}

async function saveMenuItem() {
  try {
    // Convert Unsplash URL if needed
    if (editingItem.value.image) {
      editingItem.value.image = convertUnsplashUrl(editingItem.value.image)
    }
...
async function uploadEventImage() {
  if (!eventImageFile.value) return null
  ...
  try {
    ...
    const response = await fetch('http://localhost:8000/api/events/upload-image.php', {
      method: 'POST',
      body: formData
    })
...
async function saveEvent() {
  try {
    ...
    if (eventImageFile.value) {
      ...
      if (imagePath) {
        editingItem.value.image = imagePath
      }
    }
          ...
          <v-card-text>
            ...
            >
              <template v-slot:item.image="{ item }">
                <v-avatar size="50" rounded="lg" v-if="item.image">
          ...
          <v-row>
            ...
            <v-col cols="12">
              <v-select
                v-model="editingItem.status"
                label="Status"
                :items="['pending', 'confirmed', 'preparing', 'ready', 'delivered', 'completed', 'cancelled']"
                variant="outlined"
                color="amber-darken-2"
              ></v-select>
            </v-col>
      ...
      <v-card v-if="editingItem" class="dialog-card">
        ...
        <v-card-text class="pt-4">
          ...
          <v-select
            ...
            label="Category"
            :items="['Cakes', 'Pastries', 'Desserts', 'Drinks', 'Snacks', 'Food Plans', 'Baking Classes']"
            variant="outlined"
            color="amber-darken-2"
            class="mb-2"
          ></v-select>
          ...
          <v-text-field
            ...
            variant="outlined"
            color="amber-darken-2"
            placeholder="https://images.unsplash.com/photo-... or /Menu/cake.jpg"
            hint="Enter full image URL (https://images.unsplash.com/...) or local path (/Menu/cake.jpg) - Do NOT use /public/"
            persistent-hint
            class="mb-2"
          ></v-text-field>

          <v-text-field
            v-model.number="editingItem.kg"
            label="Weight (kg) - Optional"
            type="number"
            step="0.5"
            variant="outlined"
            color="amber-darken-2"
            hint="For cakes: enter weight in kg (leave empty if not applicable)"
            persistent-hint
            class="mb-2"
          ></v-text-field>
    ...
    <v-dialog v-model="showEventDialog" max-width="600">
      <v-card v-if="editingItem" class="dialog-card">
        ...
        <v-card-text class="pt-4">
          ...
          <v-text-field
            v-model.number="editingItem.spots_available"
          ...
          <div class="mb-4">
            <p class="text-subtitle-2 mb-2">Event Image</p>
            <v-file-input
              ...
              show-size
              clearable
              hint="Upload JPG, PNG, or WebP image (max 5MB)"
              persistent-hint
            ></v-file-input>

            <!-- Image Preview -->
            <v-card v-if="imagePreview || editingItem.image" class="mt-3" elevation="2">
              <v-img
                :src="imagePreview || `http://localhost:8000${editingItem.image}`"
                height="200"
                cover
              ></v-img>
            </v-card>
        ...
        <v-card-actions class="pa-4">
          <v-spacer></v-spacer>
          <v-btn variant="text" @click="showEventDialog = false" :disabled="uploadingImage">Cancel</v-btn>
          <v-btn color="amber-darken-2" @click="saveEvent" :loading="uploadingImage">
